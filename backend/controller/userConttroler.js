...
const defaultAvatar = { public_id: "default-avatar", url: "/seed-images/default-avatar.png" };
...
exports.registerUser = asyncWrapper(async (req, res) => {
  ...
  if (hasCloudinaryConfig() && req.body.avatar) {
  ...
  const user = await userModel.create({ name, password, email, avatar });
...
exports.updateProfile = asyncWrapper(async (req, res, next) => {
  ...
  if (req.body.avatar && hasCloudinaryConfig()) {
    const user = await userModel.findById(req.user.id);
    const imageId = user.avatar?.public_id;

    if (imageId && imageId !== "default-avatar") {
